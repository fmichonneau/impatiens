...
\bigskip
\noindent CRYPTIC AND NOT-SO-CRYPTIC SPECIES IN SEA CUCUMBER SPECIES COMPLEX
...
\begin{center}
  \noindent{\Large \bf Cryptic and not-so-cryptic species in the complex
    ``\textit{Holothuria (Thymiosycia) impatiens}'' (Forsskal, 1775)
...
<<common-variables, echo=FALSE>>=
@

\subsubsection{Abstract}

Identifying accurately species is critical for our understanding of patterns of
diversity and speciation. However, for many organisms with simple and variable
...
cucumbers. We used a three-step approach to show that ``\textit{Holothuria
  impatiens}'', a common, shallow-water species, occurring across the
Indo-Pacific, the Western Atlantic and the Mediterranean Sea, targeted locally
by fisheries, is a complex of at least 13 species. (1) We used the Generalized
Mixed Yule Coalescent (GMYC) model to identify putative species without
\textit{a priori} hypotheses. In the process, we also show that the number of
putative species estimated with GMYC can be affected considerably by the priors
used to build the input tree. (2) We assessed based on coloration patterns and
distributional information, the most relevant hypothesis. This approach allowed
us to identify unambiguously 9 species. However, some of the lineages
consistently assigned to belong to different species using GMYC, are occurring
in sympatry and are not differentiated morphologically. (3) We used Bayes
factors to compare competing models of species assignment using the multispecies
coalescent as implemented in *BEAST. This approach allowed us to validate that
the species identified using GMYC were likely reproductively isolated. Estimates
of the timing of diversification also showed that these species diverged less
than 2 Ma, which is the fastest case of closely related species occurring in
sympatry for a marine metazoan. Our study demonstrates how clarifying species
limits contribute to refining our understanding of speciation.
...
Despite challenges with definitions, species remain the basic units in any
biological inquiry, such as assessing patterns of diversity, identifying regions
of conservation concern, and managing resources \citep{Pimm2014}. For most
groups, species limits are often misunderstood and the level of undescribed
diversity uncertain, making even estimates of diversity poorly constrained
...
of the processes that maintain present and future diversity can hinder these
goals \citep{Moritz2002}. Genetic data, and DNA barcoding in particular, are
facilitating the documentation of biodiversity not only by providing an
inexpensive and efficient way to identify species and resolve complexes, but
also by providing insights into their evolutionary history.

Molecular studies have revealed that many species, thought to be
well-understood, are complexes. Cryptic species are likely common, especially in
the sea, because unlike us primates, most organisms rely on chemical signaling
for species recognition \citep{Knowlton1993}. In most cases, these
``pseudo-cryptic'' species differ morphologically but in traits not
traditionally considered, or in traits not typically available to taxonomists in
preserved specimens \citep{Knowlton1993}. For instance, differences in
coloration \citep{Brown2007,Malay2010}, habitat \citep{Prada2013}, or host
preference \citep{Hebert2004}, have often been ignored as indicative of species
differences in themselves, before the abundance of cryptic lineages was realized
through molecular studies. During the past decade, single-locus DNA barcoding
has taken off as quick means of re-evaluating species limits. While deep
divergence in a single marker is not diagnostic of species limits, concordant
reciprocal monophyly in two traits does demonstrate the lack on gene flow. Thus
combined, reciprocal monophyly of typically a mtDNA marker (DNA barcoding) with
at least one other defining attribute (other loci, morphology, geographical
distribution) provides a way to infer species limits using the evolutionary
significant unit (ESU) concept \citep{Moritz1994} (see below). While this
powerful approach has led to the recognition of many cryptic species, it has had
limited efficacy in taxa that have lineages that are deeply divergent in mtDNA,
but lack other differentiating attributes. Such patterns can arise from the
fusion of previously differentiated populations, but could also be indicative of
truly cryptic species. In such cases, testing for divergence in other
(non-mitochondrial) loci provides a straightforward test of
differentiation. However, this approach has been particularly limiting because
of the slow rate of divergence or paralogy in typical ``workhorse'' nuclear loci
that be amplified by broadly universal primers, and coversely the lack of
universality in primers that use rapidly evolving markers, such as
microsatellites.

New methods based on the multispecies coalescent are emerging as powerful ways
to investigate species limits in challenging complexes where previous evidence
was equivocal. By using multi-locus datasets, these methods overcome some of the
shortcomings of single-locus analyses that were unable to distinguish between
different processes leading to identical patterns (e.g., incomplete lineage
sorting and introgression), and do not depend on arbitrary threshold of genetic
divergence. Instead, they account for the stochastic process associated with the
independent sorting of genealogies along the species tree, and use it to
estimate the species tree and the demographic history of the species
\citep{Fujita2012a}. These methods also provide a statistical framework to test
competing species delineation hypotheses \citep{Baele2012}.
...
common. These observations have led to the realization that (1) selection (e.g.,
along environmental gradients, sexual selection, host specificity) may play an
important role in reproductive isolation (reviewed in \citep{Bowen2013}); (2)
changes in species distributions can rapidly obscure the geographic context at
the time of speciation. By clarifying species boundaries, providing more
accurate estimates of the timing of speciation, and populations sizes, the
multispecies coalescent may shed light on the relative contribution of these
factors in shaping patterns of diversity, reveal new patterns and refine our
understanding of speciation in the sea.

In this study, I use color patterns, genetic, distributional and
...
Forssk\r{a}l (1775) posthumously described \textit{Fistularia impatiens} from
...
attributed to \textit{Holothuria}, the largely reef-associated and most diverse
genus within the Holothuriidae. \textit{Holothuria impatiens} is the type
species of \textit{Thymiosycia} \citealt{Pearson1915}, a subgenus of
\textit{Holothuria}. The modern concept of \textit{Thymiosycia} was proposed by
\citet{Rowe1969} to include 13 species.
% characterized by well developed, regular and symmetrical tables and buttons.


\textit{Holothuria impatiens} today is recognized as the most widespread species
in the genus, ranging from the Red Sea through the entire Indo-West Pacific, to
the East Pacific, Caribbean and the Mediterranean. It is a common, to locally
abundant species found under rocks and other cryptic habitats, and largely
restricted to shallow water (< 10~m), although it has been recorded down to
158~m (Y. Samyn, pers. comm.).  Because of its broad distribution, it is
one of the more studied species of sea cucumbers, with studies on its reproduction
\citep{Harriott1985}; Cuvierian tubules
\citep{Flammang2002,Becker+Flammang2010}; toxicity \citep{Bakus1974}; feeding
preferences \citep{Roberts1982}; parasites \citep{Martens1994}; chemical
composition, morphometrics of the shape of, and ontogenic changes in,
ossicles \citep{Hampton1958,Hampton1959,Cutress1996}. It has also been included
in molecular phylogenies that investigated relationships among the major groups
of holothurians (e.g., \citep{Lacey2005}) or as an outgroup when studying
relationships within a sub-genus (e.g.,
\citep{Honey-Escandon2012}). \textit{Holothuria impatiens} is a low-value
commercial species that is fished at least in the Eastern Pacific
...
approaches outlined by \citep{Leache2010,Satler2013,Grummer2014}. First, I used
...
\citep{Pons2006,Monaghan2009}. I then used independent lines of evidence (color
...
\citep{Heled2010,Baele2012} to evaluate species limits. This study shows that
...
Caribbean, Red Sea, tropical Indian and Pacific Oceans
(Table~\ref{tab:specimen-table}).


\subsection{DNA extraction and amplification}

<<ambiguities, echo=FALSE>>=
@
...
sometimes from tube feet, gonads, longitudinal muscles or body wall. When
possible, the extractions were performed on tissue sampled in the field.
...
(\Sexpr{round(nAmbPositions/dim(impAlg)[2], 4)*100}\% of the alignment length,
...
\item ``Evolutionary significant units (ESUs)'' are defined as reciprocally
  monophyletic lineages in one marker (COI in this study) that are also
  characterized by another independent defining attribute (e.g., coloration,
  morphology, geographical distribution, reciprocal monophyly in another
  marker). We use this concept as defined by Moritz \citep{Moritz1994}.
\item The term ``species'' is used in this study to denote lineages supported by
  the model of species delineation that best explains the data as assessed by
  Bayes factors estimated from the multispecies coalescent in *BEAST. They
  provide an indirect assessment of reproductive isolation and the species are
  assumed to not exchange genes, forming independent distinct lineages.
\end{itemize}
...
(\citep{Monaghan2009, Fujisawa2013, Talavera2013}), we here revisit the impact
of specifying a strict or a log-normal relaxed clock, as well as the type of
tree prior used (coalescent with constant population size \citep{Kingman1982},
coalescent with exponentially growing population size \citep{Griffiths1994}, and
Yule \citep{Yule1925}). The type of clock used affects branch lengths, it is
...
cannot accommodate for null branch lengths associated with identical sequences
\citep{Fujisawa2013}. Contrary to other programs, BEAST does not assign zero
edge length to terminal branches for identical sequences, and thus allows
...
% GP: not sure what you mean by that.  Since Galap and EP are sisters, they are
% both equally sister to the WA species

% GP: [We discussed this earlier, but I forget the details.  This does not read
% well without more detail.  You say 3 Ma is last contact, then use 7 Ma for
% divergence without justification.  Your confidence interval does not include 3
% Ma…]
...
\citep{Lessios2008,Bacon2015}. We chose a conservative prior to calibrate the
clock as the presence of two species in the Eastern Pacific (\texttt{Gala} and
\texttt{EP}, Fig.~\ref{impatiens-tree}) suggests that the vicariance event
associated with the raise of the isthmus of Panama happened earlier rather than
later. We constrained the age of the node corresponding to the most recent
common ancestor of the individuals found in the Galapagos archipelago, the coast
of the Eastern Pacific and in the Caribbean using a log-normal prior with a
\textit{log}(mean) of 1.5, a \textit{log}(standard deviation) of 0.75 and an
offset of 2.5. This distribution translates into a mean age of of divergence of
\Sexpr{round(exp(1.5),2)} millions years ago (Ma), (95\% confidence interval of
[\Sexpr{paste0(round(qlnorm(c(0.025, 0.975), meanlog=1.5, sdlog=0.75) + 2.5, 2),
  collapse=", ")}]).
...
\subsection{Species limits, coloration, and geography}
...
Coloration was obtained from photographs of the live specimens and from field
observations. Habitat, habit and location were retrieved from collecting
...
clusters, have distinct ranges, but cannot be differentiated morphologically. A
notable cluster includes the widespread putative species (ESU1) whose range
overlaps with that of two related putative species (\texttt{Hawaii},
\texttt{Wpac}) and borders a third (\texttt{RedSea}). To test whether these four
putative species are reproductively isolated, we compared competing hypotheses
of species delineation with *BEAST v 1.8.0 \citep{Heled2010} using Bayes factors
...
\ref{tab:delineationModels}). We included the more distant putative species
\texttt{ESU3} and \texttt{gracilis} in our analysis to allow the estimation of
the topology when all individuals for the species tested where attributed to
\texttt{ESU1}. The null hypothesis was that all putative species were
reproductively isolated.

<<star-beast-stats, echo=FALSE>>=
@

In these analyses, we selected individuals for which we had sequences from at
least one mitochondrial marker and one nuclear marker
(\Sexpr{length(sBeastTaxa)} individuals: \Sexpr{sBeastESU["ESU1"]} of
\texttt{ESU1}, \Sexpr{sBeastESU["Hawaii"]} of \texttt{Hawaii},
\Sexpr{sBeastESU["Wpac"]} of \texttt{Wpac}, \Sexpr{sBeastESU["RedSea"]} of
\texttt{RedSea}, \Sexpr{sBeastESU["gracilis"]} of \texttt{gracilis},
\Sexpr{sBeastESU["ESU3"]} of \texttt{ESU3}). We used data from the following
markers: 16S, COI, ATP6, c0036, c0775, ITS, LSU and H3a; and the same approach
as for the phylogeny of the entire complex (see below): using Gblocks to remove
ambiguous parts of the alignment in ITS, PartitionFinder to find the most
appropriate partition scheme and models of molecular evolution. We unlinked
clocks for all markers, used strict molecular clocks, fixing the COI clock at
1\%.million years$^{-1}$, the rate estimated from the analyses on COI used for
the GMYC analyses. The rate for the other clocks were estimated relatively to
the COI clock. We ran at least two independent runs for 50.10$^{6}$ generations
for each group.
...
<<loci-coverage-data, warning=TRUE, echo=FALSE>>=

@


The complete sequence data set was \Sexpr{sum(locChar["bp_alg", ])}~bp (with
\Sexpr{sum(locChar["bp_alg", c("16S", "COI", "ATP6")])}~bp from mitochondrial
genome, and \Sexpr{sum(locChar["bp_alg", c("c0036", "c0775", "H3a", "ITS",
  "LSU")])}~bp from the nuclear genome,
Table~\ref{tab:loci-characteristics}). Of the \Sexpr{sum(locChar["S_i",])}
parsimony-informative sites, \Sexpr{round(sum(locChar["S_i", c("16S", "COI",
  "ATP6")])/sum(locChar["S_i",])*100, 0)}\% were from the mitochondrial loci
(Table~\ref{tab:loci-characteristics}).


<<loci-characteristics-data, results="asis", echo=FALSE, warning=FALSE>>=
generate_loci_table(locChar)
@


We obtained sequence data from \Sexpr{length(unique(locus_graph_data[,
...
sequenced for at least one mitochondrial locus,
\Sexpr{round(length(unique(hasMtNuc[,
  "Extract"]))/length(unique(locus_graph_data[, "Extract"]))*100, 0)}\% were
sequenced for at least 1 nuclear locus, and
\Sexpr{round(sum(table(hasMtNuc[, "Extract"]) >=
  2)/length(unique(locus_graph_data[, "Extract"])) *100, 0) }\% for 2 or more
...
<<gmyc-coi-results, eval=TRUE, echo=FALSE>>=
@

<<gmyc-coi-vars, eval=TRUE, echo=FALSE>>=
minSingle <- min(subset(gmyc_summary, analysisType == "single")$mean)
maxSingle <- max(subset(gmyc_summary, analysisType == "single")$mean)
minMulti <- min(subset(gmyc_summary, analysisType == "multi")$mean)
maxMulti <- max(subset(gmyc_summary, analysisType == "multi")$mean)
stopifnot(all(subset(gmyc_summary, analysisType == "single" &
                       clock == "strict" & sequences == "allSeq")$mean == 16))
stopifnot(all(subset(gmyc_summary, analysisType == "single" &
                       clock == "strict" & sequences == "noDup")$mean == 17))
allSeqStrictSingle <- gmyc_summary[gmyc_summary$analysisType == "single" &
                                     gmyc_summary$clock == "strict" &
                                     gmyc_summary$sequences == "allSeq", "mean"]
noDupStrictSingle <- gmyc_summary[gmyc_summary$analysisType == "single" &
                                    gmyc_summary$clock == "strict" &
                                    gmyc_summary$sequences == "noDup", "mean"]
@

The maximum-likelihood estimates for the number of putative species varied from
\Sexpr{minSingle} to \Sexpr{maxSingle} for the single-threshold method (sGMYC),
and from \Sexpr{minMulti} to \Sexpr{maxMulti} for the multi-threshold method
(mGMYC) (Fig~\ref{fig:gmyc-coi-plot}).
...
divergent haplotypes within the \texttt{ESU3} when using the Yule prior.
...
With the strict clock analyses, the single additional species delineated between
the unique haplotypes and all sequences represents the separate recognition of
\texttt{tiger} and \texttt{tigerRedSea}.

In all analyses, at least one putative species was represented by a single
...
Despite these common characteristics, the combination of color patterns and
geographical distribution allows us to distinguish unambiguously at least ten
species (Fig.~\ref{fig:species-limits}, \ref{fig:photo-ESU1-andFriends},
\ref{fig:photo-ESU2-andFriends}). \texttt{Medit}, \texttt{WA}, \texttt{Gala},
\texttt{EP}, \texttt{ESU1} have similar color patterns but subtle differences,
and the restriction of their ranges to an oceanic basin or an archipelago allow
separation. \texttt{ESU2} is the only species with bright yellow papillae and
tentacles, but is otherwise quite variable in coloration
...
reach regularly more than 30~cm. They live in deep crevices on fore reefs from
which they partially emerge at night, with their posterior remaining anchored;
this makes them challenging to collect. \texttt{gracilis} has very large,
conical, typically red tubercles, and a very distinctive ``gritty'' texture.
\texttt{ESU3} has elongated, pyramidal tubercles with white papillae. However,
\texttt{ESU1}, \texttt{Hawaii} and \texttt{Wpac} are morphologically
indistinguishable and have broadly overlapping distributions
...
<<starbeast-summary, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE>>=
@
...
% GP: [This section needs a lead sentence summarizing what follows]
...
The model that included all putative species identified by GMYC represented by
more than one individual, and that do not differ morphologically
(\texttt{Hawaii}, \texttt{Wpac} and \texttt{ESU1}) had the highest marginal
likelihood (Figure~\ref{fig:starbeast-summary}). The Bayes factors showed ``very
strong'' support for the null hypothesis when compared with the models that
considered each of \texttt{Hawaii}, \texttt{Wpac} and \texttt{RedSea} in turn
panmictic with \texttt{ESU1} (BF=\Sexpr{BFHawaii}, \Sexpr{BFWpac}, and \Sexpr{BFRedSea}
respectively). The model with an additional ``ghost'' species also indicated a
poorer fit (BF = \Sexpr{BFsplit}), indicating that this approach did not favor a
model that included more species. The model with a random assignment of the
species had the worst fit (BF = \Sexpr{BFrandom}).
...
likelihood were however smaller, BF = \Sexpr{BFHawaiiNoCOI},
\Sexpr{BFWpacNoCOI}, \Sexpr{BFRedSeaNoCOI}, for models that considered
\texttt{Hawaii}, \texttt{Wpac}, and \texttt{RedSea} as panmictic with
\texttt{ESU1}, indicating ``strong'' to ``very strong'' support in favor of the
null hypothesis.

Analyses conducted on nuclear data alone led to low BF values, that did not
support the null model (BF = \Sexpr{BFHawaiiNoMt} for no \texttt{Hawaii}, and
...
threshold to consider the evidence ``strong'' (BF = \Sexpr{BFWpacNoMt}).
...
% GP: [This section needs a lead sentence summarizing what follows]
The phylogenetic relationships recovered with BEAST and RAxML on the
concatenated alignment were identical (Fig.~\ref{fig:impatiens-tree} and
...
single basins (Mediterranean, Western Atlantic, Red Sea), or archipelagoes
(Hawaii, Galapagos) but others have ranges that encompass large sections of the
Indo-West Pacific. This diversity of distributions results in the co-occurrence
of several species in most parts of the Indo-West Pacific, peaking with at least
5 species in the Philippines. At least 3 species broadly co-occur across the
northern tropical Pacific basin (from the Philippines to the Hawaiian
archipelago).

Species in the Indo-West Pacific have overlapping ranges, even for those that
diverged relatively recently. For instance, \texttt{Wpac} and \texttt{Hawaii}
co-occur with \texttt{ESU1} from which they diverged and diverged (1.8~Ma and
2.2~Ma respectively, Fig.~\ref{fig:impatiens-tree} and
...
\begin{figure}
  ...
  \caption{Distribution maps of the ESUs in Eastern Pacific and Caribbean (WA,
    Gala, EP) based on sequenced individuals.}\label{fig:impatiens-map-WA}
\end{figure}

\begin{figure}
  {\centering \includegraphics[width=\maxwidth]{figures-remake/impatiens-map-group1.pdf}}

  \caption{Distribution map for ESU1, ESU3, gracilis, Hawaii, RedSea, Wpac based
    on sequenced individuals.}
...
\begin{figure}
  ...
  \caption{Distribution map for ESU2, tiger, and tigerRedSea based on sequenced individuals.}
...
Species delineation is a complex issue and results can depend on the characters
and methods chosen. Here, using complementary approaches, combining discovery
(GMYC) and testing methods (*BEAST), we unraveled at least 13 species within the
complex ``\textit{Holothuria impatiens}'' supported by multiple lines of
evidence.

Traditional detailed taxonomic investigations failed to capture the diversity
unraveled by molecular analysis in this complex. For most putative species thus
identified, variation in traits not typically considered to discriminate closely
related species in sea cucumbers arose as being being diagnostic in the light of
genetic data. The consideration of geography and differences in color patterns
allowed the recognition of 10 species in the complex in retrospect
(Figure~\ref{fig:species-limits}). Taxonomic studies for most marine
...
to see, as color patterns are frequently lost or fade with time. Because hard
parts are easily preserved, are often divergent among related species, and their
form is generally not impacted by preservation, they tend to be favored for
differentiating species. In sea cucumbers, species-level taxonomy has relied
almost entirely on the shape of ossicles, microscopic calcareous secretions
found in many tissues, because ossicles show substantial diversity and
variability, whilst variation in the simple anatomy of these animals is limited
at lower taxonomic levels. Even with ossicles, workers have mostly focused on
those found in the body wall, ignoring ossicles in internal structures, until
recent decades. Little attention has been given to intraspecific variation,
often ossicle assemblages are described from only a single individual. This, in
combination with the fact that related species often co-occur in the same
habitat, has contributed to the failure to recognize and differentiate species
in this complex.

Mitochondrial data has rapidly shown that several species are currently lumped
under \textit{Holothuria impatiens}, but the exact number of species and their
boundaries are more complex issues, in particular in groups which little about
their biology is known. The discovery methods, based on genetic differentiation
in COI, delineated at least 13 putative species. Ten of these were supported by
morphological (color) or broad geographical evidence, while three were not so
distinguishable. Testing methods supported the remaining three as
genetically-isolated, cryptic species, corroborating the existence of at least
13 species. This study includes additional putative species whose status could
not be fully evaluated given that they were represented by a single individual
(see below).
...
method to factors affecting the shape of the tree used have not been
investigated thoroughly. Tree shape are affected by biological factors and by
the method used to build the tree. Simulated data have been used to test the
influence of diversification rates, effective population sizes, and migration
rates, on the accuracy and precision of GMYC estimates \citep{Papadopoulou2008,
  Esselstyn2012}. Some of the effects of the methodological choices have been
evaluated \citep{Pons2006, Monaghan2009, Fujisawa2013, Talavera2013}, but it is
difficult to generalize as its impacts depend in part on the nature of the
dataset.

The single-threshold method (sGMYC) provided more accurate estimates of the
number of species included in our sample compared to the multiple-threshold
method (mGMYC). Species delineated using sGMYC were corroborated by the
integrative taxonomic and multigene coalescent methods. In contrast, mGMYC
oversplit lineages supported by other lines of evidence. The lower accuracy of
mGMYC was also observed by the authors of the method using simulated datasets
\citep{Fujisawa2013}. In their study, they observed that the 95\% confidence set
for the estimated number of putative species using sGMYC and mGMYC overlapped
\citep{Fujisawa2013}. Here, overlap between the sGMYC and mGMYC estimates of was
only observed when the analysis was performed on haplotypes but not when it was
performed on all sequences.

Performing the analysis on all sequences yielded to more accurate estimates of
the number of putative species, and made the results were more consistent
regardless of the prior chosen. The number of putative species obtained using
sGMYC with all sequences was consistently 16, except for the relaxed clock with
the Yule prior where it was 14. The 16 species delineation was the most
realistic as it recovered all morphologically distinct
ESUs. \citet{Talavera2013} found that using all sampled haplotypes did not
impact the number of species estimated using GMYC compared to using only unique
haplotypes. In contrast, here we confirmed the theoretical expectation that
removing duplicated sequences led to an overestimation of the number of species
and decreased the precision of the estimates if the tree was estimated with
BEAST. Removing identical sequences in the analysis overestimates the effective
population size parameters that BEAST uses to infer the coalescence times for
the lineages. In turn, these spurious increases coalescence times make the
estimation of the inflection point between species- and population-level
coalescent events that GMYC uses earlier in the tree, and more difficult to
detect. This bias leads to the oversplitting of lineages and decreases
accuracy. Investigating how the number of individuals sampled per species
increases the accuracy of the delineation could be worth investigating.
% Esseltyn+2012 also used both all sequences & only unique, but didn't provide
% the differences in results


The results of this study, in concert with previous studies, highlight that the
most appropriate combination of priors and parameters to use in GMYC analyses
depends on the dataset. \citet{Monaghan2009} obtained the most accurate results
when using a coalescent prior and a relaxed log-normal clock on unique
haplotypes (but they did not perform the analysis using all sequences), and
concluded that the Yule prior led to an overestimation of the number of species
for 4 of the 5 taxonomic groups they investigated. On the other hand,
\citet{Talavera2013} did not find any notable differences in the number of
species estimated using trees estimated with different sets of priors. With this
dataset, we obtained the most accurate results when we used sGMYC on all
sequences, and the best precision when using a relaxed log-normal clock and a
coalescent prior with exponential population growth. However, the most
appropriate set of parameters to use when performing a GMYC analysis will be
influenced by the number of individuals included, how divergent the species are,
and the heterogeneity in the substitution rate among the lineages included. In
addition, the species to be delineated need to be fully sorted into reciprocally
monophyletic lineages to make the GMYC approach suitable.
...
environmental samples \citep{Pons2006}. In this context, a reasonable amount of
inaccuracy or imprecision from the true number of species may be acceptable. The
advantage of the method, namely providing a rapid and objective assessment of
the number of species, outweighs the possible deviations from the true
value. However, in the context of a taxonomic study where the goal is to
identify species limits among a few hundred individuals, factors affecting tree
shape need to be considered, and additional lines of evidence need to be
included to validate the putative species delineated \citep{Marshall2011a,
  Esselstyn2012}. More generally, any study that uses GMYC to estimate species
boundaries among a diverse group of samples should run the analysis using
several methods that produce robust branch-length estimates.

% GP: [Would be good to articulate the best way to use GMYC for metabarcoding
% type efforts – spell out the best practice in terms of priors, replicate
% haplotypes, thresholds etc.  Spelling out of best practice, and putting this
% in abstract too will be useful to readers]


% GP: [highlighted section is about minor issue and needs to move down (and
% ideally, streamlined) in discussion.  Discussion should start with the big
% bang results, not these minor unresolved details]
sGMYC recovered more putative species than we can confidently recognize, as it
included exemplars represented by single individuals in which variation cannot
be evaluated. This is a common challenge in single-locus species delineation
methods, where the broad geographical sampling of many individuals often reveal
unique, divergent haplotypes that may not represent different species.  In
particular, three individuals characterized by divergent haplotypes, related to
\texttt{ESU1} for one, and to \texttt{ESU3} for the others, were consistently
recovered as separate species. In both cases, a single individual was available,
making it impossible to assess reproductive isolation with *BEAST. The
individual related to \texttt{ESU1} (UF8191) had the typical color pattern of
\texttt{ESU1}, with which it co-occurs and no line of evidence seem to indicate
that it could be a different species. The same situation occurs with one of the
individuals related to \texttt{ESU3} (N0027) that cannot be differentiated from
other individuals from \texttt{ESU3}. The other individual related to
\texttt{ESU3} (RUMF-ZE-0074) is small, and was dredged from 32~m in a soft
bottom bay of Kume Island, Ryukyus, Japan, a different habitat from the
typically reef-associated \texttt{ESU3}. The distinct ecology and deeply
...
% Other species delineation methods using sequence data are also available
% (e.g., RESL \citep{Ratnasingham2013}, ABGD \citep{Puillandre2012a}, bPTP
% \citep{Zhang2013}, bGMYC \citep{Reid2012}), and it would be interesting to
% investigate how they perform compared to the results obtained here.
...
support for these species. Here, COI (the fastest evolving marker in this study)
was used to assign species to individuals for putative species that could not be
differentiated morphologically.

Assigning species based on their differentiation on the fastest evolving
mitochondrial marker available could bias the results in two ways. First,
because the marker evolves faster than the other loci used, the information
carried by this locus could potentially overwhelm the signal contained in the
other loci, and thus the results of the model comparison will strongly favor the
results suggested by the fastest evolving marker. Second, the COI genealogy and
by extension the mitonchondrial markers might not accurately represent species
limits because of introgression among closely related lineages within the
complex. This could be reinforced by the fact that the mitochondrial markers
will evolve on average faster than the nuclear loci, and therefore reinforce the
delineation hypothesis put forth by the fastest evolving marker. We attempted to
evaluate the relative contribution of these confounding effects by conducting
the analyses without COI and without mitochondrial data respectively. % add
%% sentence about predictions
The results for the analyses conducted without COI were qualitatively similar to
the analyses performed with this locus, but the statistical evidence in favor of
the hypothesis considering these closely related lineages as different species,
was weaker. When the analysis was conducted on nuclear data alone, the results
were not as consistent but nonetheless supported the existence of several
reproductively isolated lineages among these morphologically indistinguishable
species. The lack of support for the null hypothesis was likely the result of
insufficient differentiation among the putative species for the nuclear
loci. These results suggest that the remaining of the mitochondrial loci used in
our study also favor the null hypothesis, but additional independent nuclear
loci with enough differentiation to tease out these species are needed.

% Although we have no evidence supporting this here, introgression and incomplete
lineage sorting...

Other confounding issues with assigning individuals to species based on COI
sequences are introgression and incomplete lineage sorting. % GP: need to
%% discuss introgression if you mention it
Recently diverged species might still be poly- or paraphyletic in their
COI genealogy making species assignment inaccurate.
%% [this is not about inaccurate species assignment per se, but lumping young
%% species into a single species], GP comment shows that I need to clarify the
%% context of species assignment in this case.
Inaccurate species assignment can potentially be detected from the population
sizes estimated by *BEAST that will be unusually high \citep{Leache2013,
  Drummond2014}. This would however require to analyze more loci than in the
present study to obtain accurate estimates of population sizes \citep{Heled2010,
  Harris2013}. Determining how the Bayes factors will behave if the null
hypothesis is misspecified remains to be investigated. %% [context of last
%% sentence unclear to me.  This paragraph needs work.  Not sure if my edit
%% capture what you are trying to say]

\subsection{Species limits and coloration}

Before molecular methods have called attention to coloration being relevant for
differentiating closely related species, marine invertebrate taxonomists have
generally been reluctant to consider species defined solely by differences in
coloration \citep{Knowlton1993}. Although variation in coloration within
``\textit{H. impatiens}'' has been noticed by previous workers, and some unusual
color morphs were described as sub-species by Clark \citep{Clark1921,
  Clark1938}, the focus on ossicle shape to distinguish sea cucumber species has
led to an underestimation of species diversity.
...
\citep{Knowlton1993}, fish \citep{McMillan1999}, mollusks
\citep{Meyer2005-evolution}. In some of these cases, the association between
color patterns and species limits is maintained by assortative mating (fish
\citep{McMillan1999}, crabs \citep{Detto2007, Baldwin2012}). Given that most sea
cucumbers inhabit cryptic habitats and do not possess image-forming eyes, the
mechanisms maintaining a tight association between coloration and species
identity through assortative mating is unlikely.
%% discuss other potential forces that may shape it – predators (mimicry),
%% accidental (neutral evolution), etc.

While some species of ``\textit{H. impatiens}'' have distinctive aspects to
their coloration, others do not and are polymorphic. For instance, \texttt{ESU2}
has two color morphs: a common color pattern that occur throughout the range of
the species, and one restricted to the Western Indian Ocean (Madagascar and
R\'{e}union Island). Both \texttt{gracilis} (alternate morph in the southern
part of the Great Barrier Reef) and \texttt{ESU1} (alternate morph found in
Lizard Island, Australia; Hawaii; and Guam) are also polymorphic.
Interestingly, these alternate color morphs are similar for the three species
(uniform chocolate brown background with white to yellowish tubercles), and
...
%% [paragraph needs work.  Several things confounded – typical vs atypical,
%% variable vs not variable, diagnostic vs not diagnostic, geographically
%% homogeneous vs allopatric color forms]

% TODO -- add pictures of alternate color morphs



Even though alternate color patterns in these species
%% [which ones – several are variable, so need to be clear)
are not genetically distinguishable with the markers used in this study, these
color morphs may represent incipient species.  Studies in fish indicate that
color differentiation can precede genetic divergence in commonly surveyed
markers (e.g., \citep{McMillan1999, Gaither2014}). Additional genetic data
(microsatellites, SNPs) can test whether they represent distinct evolutionary
lineages.


\subsection{Geographical distributions}

The geographical ranges of the species in this complex vary considerably, and a
diversity of processes seem to be responsible for their present
distributions.

Interestingly no speciation along a continental margin is observed in the
complex. %% this sentence is out of context.  You need to present background for
         %% the reader if you want to discuss this]

The close phylogenetic relationship of \texttt{WA} with \texttt{EP} and
\texttt{Gala} indicates that American species in the complex form a clade, which
has in part diversified through the emergence of the isthmus of Panama. The
differentiation of continental and Galapagos populations into separate species
parallels a similar divergence in the sea urchin genus \textit{Eucidaris}
% \citep{Lessios}. Additional sampling is needed to assess the range of
\texttt{Gala}. Both the insular \textit{E. galapagoensis} (Galapagos, Cocos,
Clipperton) and continental \textit{E. thouarsi} range broadly. The range of the
former spans broader open ocean barriers within its range than the barriers tha
separate it from the mainland \textit{E. thouarsi}, a distributional pattern
suggestive of ecological speciation driven by oceanic-continental habitat
differences.

Three species are largely endemic to marginal seas, suggestive that basinal
isolation may have been responsible for their origin. However, the estimated
ages of all three (\texttt{Medit}, \texttt{RedSea}, \texttt{tigerRedSea})
species are older (8.4-10.5 My, 1.1-1.7 My and 3.2-4.1 My respectively) than the
last salinity crises characterizing these seas (\textit{c.} 5 My for the
Mediterranean \citep{Hernandez-Molina2014} and potentially as late as
\textit{c.} 19,000 years ago for the Red Sea). This, in combination with their
occurrence outside their basins (\texttt{Medit} is known from Portugal and the
Canaries; and both Red Sea species from Djibouti) indicate that these species
invaded these seas recently and found refugia at the periphery of these basins
during the salinity crises \citep{DiBattista2013}. It is also interesting to
note that the age of divergence for the two species from the Red Sea do not
coincide suggesting that different events were responsible for the geographic
...
reported here. %% [This paragraph is a bit loose in context here.  Undersampling
               %% is clearly an issue for all taxa]

For species with wide ranges the geographic context of their divergence are less
obvious, and current distributions may not reflect species distributions at the
time of speciation. The two most widespread species (\texttt{ESU1} and
\texttt{ESU2}) are sympatric or parapatric with their closely related species,
and both form the terminal branches in their respective radiations. This pattern
suggests that these species were the sequential source of peripheral species
\citep{Malay2010}.

The lack of genetic differentiation between the Indian and Pacific Ocean
populations for species whose distribution spans the Indo-Pacific barrier (IPB)
is noteworthy. The IPB results from the restriction of the seaway between the
Sunda and the Sahul shelves. During glaciation events, when sea levels were
100+~m below present level, only a narrow channel connected the Indian and
Pacific Oceans. In a recent literature survey, 15 out of 18 species of fish and
invertebrate investigated showed genetic differentiation across the IBP
\citep{Gaither2010}. Additionally, the genetic diversity of \texttt{ESU1} is
low: among the 61 individuals sequenced, we found a total of 18 haplotypes, with
the 3 most common haplotypes found in 40 individuals. Finally, both Marquesas
and Oman, characterized by distinct ecological conditions from the rest of the
Indo-Pacific hypothesized to increase the speed and probability of divergence,
harbor the most widespread haplotype suggesting recent colonization of these
peripheral areas to \texttt{ESU1}'s range. Together, these lines of evidence
suggest a recent geographical expansion for the widespread species in the
complex, but more analyses to confirm this hypothesis are needed.
...
after diverging in geographical isolation is key to understanding the temporal
dynamics of local diversity.
...
attainment of secondary sympatry than species associated with insular habitats
in both vertebrates (e.g., \citep{Taylor2005, Hodge2012}), and invertebrates
(e.g., \citep{Meyer2005-evolution, Malay2010, Williams2011}). As a result,
closely related species typically show strict allopatric distributions in
oceanic setting (e.g., as in \citep{Meyer2005-evolution}) but often occur in
sympatry along continental margins (e.g., \citep{Williams2011}). Differences in
rates of secondary sympatry vary across taxa but are faster in continental
settings. For instance, in the wrasse \textit{Anampes} sympatric sister species
in continental setting diverged 600 ka ago, while the most closely related
sympatric species in oceanic setting shared a common ancestor more than 10 Ma
ago \citep{Hodge2012}. In the turbinid \textit{Lunella}, none of the 7 lineages
...
knowledge, the fastest documented examples of speciation for a broadcast-spawning
invertebrate in an oceanic setting (\texttt{ESU1} and \texttt{Hawaii} diverged
less than 2~Ma, and \texttt{ESU1} and \texttt{Wpac} about 2.5~Ma).
%%  [can you generalize to marine animal?  Any fist faster?  Also, how does Echinometra compare?]
Thus, these species have been able to evolve reproductive isolation rapidly, and
differ sufficiently in their ecology to allow co-existence. In other words, the
temporal dynamics of diversification in some oceanic species of the
``\textit{Holothuria impatiens}'' complex are as rapid as those observed for
taxa occurring along continental margins.
...
from sea urchins \citep{Lessios2011} and sea stars \citep{Patino2009} indicating
that it is plesiomorphic in the clade that sea cucumbers emerged from. They
could explain how species in the ``\textit{H. impatiens}'' complex acquired
reproductive isolation quickly. %% [last sentence too vague – you need to explain why]
...
environmental conditions render them maladapted to new islands.
%% [more importantly genomes of rare migrants will be swamped out if they
%% interbreed readily].
This model implies niche conservatism: species are narrowly adapted to a
particular environment. The predictions of this model would suggest that
\texttt{ESU1} has broadened its niche to allow the colonization of the range of
\texttt{Hawaii} and \texttt{Wpac}.
%%% [We should talk about this.  I feel the main issue is what happens when
%%% populations meet, not so much the ecology.  In a continental setting two
%%% populations meet en force, so that species boundaries are continually tested
%%% – this favors evolution of reproductive character displacement (through
%%% GRPs) and leads to rapid secondary sympatry.  In oceanic settings rare
%%% colonists, when not yet sufficiently isolated, would get swallowed by
%%% interbreeding into the receiving pop.  A rescue of this (in
%%% cukes/echinoderms?) could be is there is strong sexual selection driving
%%% rapid GRP divergence, so they arrive unable to interbreed…]
...
%% [I would cut this section, and focus on getting all this into the abstract in
%% a well written manner.  Also start the story with the method and species
%% delimitation part and have the cuke story in a secondary position]
...
\begin{figure}
  ...
  \caption{Maximum-likelihood phylogeny for all concatenated loci estimated
    using RAxML with a GTR+G model for each partition. Black circles represent
    bootstrap values $\geq$ 90, red circles bootstrap values $\geq$ 80. Colored
    vertical bars represent consensus ESUs.}\label{fig:raxml-tree}
\end{figure}
...
\begin{figure}
  ...
  \caption{Position of the species delineated with sGMYC on COI genealogies
    estimated from all sequences using various priors with BEAST. Each color
    corresponds to a different putative species. The vertical red dashed line
    corresponds to the position of the threshold associated with the highest
    likelihood, the vertical gray dotted lines corresponds to the position of
    the confidence interval for the threshold.}\label{fig:gmyc-tree-plot-allSeq}
\end{figure}

\begin{figure}
  {\centering \includegraphics[width=\maxwidth]{figures-remake/gmyc-tree-unique-sequences.pdf}}
  \caption{Position of the species delineated with sGMYC on COI genealogies
    estimated from unique haplotypes using various priors with BEAST. Each color
    corresponds to a different putative species. The vertical red dashed line
    corresponds to the position of the threshold associated with the highest
    likelihood, the vertical gray dotted lines corresponds to the position of
    the confidence interval for the threshold.}\label{fig:gmyc-tree-plot-uniqSeq}
\end{figure}
