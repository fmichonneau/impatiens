...
complex in retrospect (Figure~\ref{fig:species-limits}). Taxonomic studies of
...
depend in part on the nature of the dataset. Other species delineation methods
using sequence data are also available (e.g., RESL \citep{Ratnasingham2013},
ABGD \citep{Puillandre2012a}, bPTP \citep{Zhang2013}, bGMYC \citep{Reid2012}),
and it would be interesting to investigate how they perform compared to the
results obtained here.

The multiple-threshold method (mGMYC) led to higher estimates of putative
species, and was less accurate than the single-threshold method (sGMYC). Species
delineated by sGMYC were corroborated by the integrative taxonomic and multigene
coalescent methods. In contrast, some of the putative species proposed by mGMYC
were split lineages not obtained by the other methods. The lower accuracy of
mGMYC was also observed by the authors of the method using a simulated
dataset. They however found that the 95\% confidence set for the estimated
number of putative species between sGMYC and mGMYC were overlapping
\citep{Fujisawa2013}. We observed this pattern on the analyses using only unique
haplotypes but not in the analysis that included all sequences.
% last two sentences need work.

The number of putative species obtained using sGMYC with all sequences was
consistently 16, except for the relaxed clock with the Yule prior where it was
14. The method was therefore relatively robust to prior specification on our
dataset. The 16 species delineation is the most realistic as it recovers
morphologically differentiated species that were not considered distinct in the
14 species hypothesis (\texttt{tiger} and \texttt{tigerRedSea}). %unclear
...
unique, divergent haplotypes that may not represent different species.

%% This sentence needs to be moved somewhere else as it's not related to
%% divergent haplotypes but is more general
However, in our analysis, it is worth noting that all our recognized species
were apparently reciprocally monophyletic making the GMYC approach suitable.

The combination of priors used in our study led to overestimating the number of
species when it was tested by the authors of the method
\citep{Monaghan2009}. %unclear
On the other hand, Talavera et al \citep{Talavera2013} did not find any notable
...
more individuals per species. % this paragraph is difficult to follow

% Esseltyn+2012 also used both all sequences & only unique, but didn't provide
% the differences in results

Talavera et al \citep{Talavera2013} found that using all haplotypes vs only
unique haplotypes did not impact the number of species estimated with GMYC. In
contrast, we confirmed the theoretical expectation that removing the duplicated
sequences led to an overestimation of the number of species and decreased the
precision of the estimates if the tree was estimated with BEAST. Removing
identical sequences in the analysis overestimates the effective population size
parameters that BEAST uses. In turn, these overestimated population sizes
spuriously increase the time to coalescence for the lineages, making the
estimation of the inflection point between species- and population-level
coalescent events earlier in the tree, and more difficult to detect. This bias
leads to oversplitting and decreases accuracy. Investigating how many
individuals need to be sampled to obtain accurate estimates would be worth
pursuing.
...
statistical framework. % GP: awkward last sentence, rework as above
...
In particular, three individuals characterized by divergent haplotypes, related
to \texttt{ESU1} for one, and to \texttt{ESU3} for the others, were consistently
