...
\caption[Bayes factors for alternate models of species assignments for putative
species related to \texttt{ESU1}.]{Bayes factors comparing the null hypothesis
  (M0: \texttt{ESU1}, \texttt{RedSea}, \texttt{Wpac}, \texttt{Hawaii} are all
  different species) to alternate models of species assignment: additional
  ``ghost" species added (oversplit), no \texttt{Hawaii} (M5), no \texttt{Wpac}
  (M4), no \texttt{RedSea} (M6), no \texttt{Wpac} and no \texttt{Hawaii} (M3),
  no \texttt{noWpac} and no \texttt{RedSea} (M2), all individuals attributed to
  ...
  \cite{Kass1995}.}\label{fig:starbeast-summary}
\end{figure}
