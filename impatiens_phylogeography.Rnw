...
The goal of this study is to identify cryptic species in the
``\textit{Holothuria impatiens}'', and to understand the temporal and spatial
dynamics of its diversification. To this end, we sampled the entire known
geographic range of this complex, and assembled a multi-locus dataset. 
...
approaches by \cite{Leache2010,Grummer2013,Satler2013}. First, we used the
Generalized Mixed Yule Coalescent method (GMYC) to delineate putative species
...
appear to be morphologically indistinguishable. The multispecies coalescent
offers a statistical framework that provides an objective assessment of species
limits \cite{Fujita2012a}, even when no other morphological, ecological or
geographical information allows to identify independent evolutionary
lineages. Our study shows that these methods can be added to the lines of
evidence typically used in integrative taxonomy, and provide a powerful tool for
evaluating species limits in rapidly evolving groups with limited morphological
and genetic differentiation.

To investigate the evolutionary history and diversification of species within
the ``\textit{H. impatiens}'' complex, we built a multi-locus, time-calibrated
phylogeny.
...
retained (Table~\ref{tab:voucherInfo} for details).

We examined \Sexpr{nrow(impDB)} specimens morphologically and \Sexpr{nrow(impAlg)} were
used for molecular analyses. These specimens were collected across the entire
known range of \textit{H. impatiens}: Mediterranean Sea, Caribbean Sea, Red Sea,
tropical Indian and Pacific Oceans (Table~\ref{tab:voucherInfo}).
...
sequences, only haplotypes) favored to assign each codon position to independent
partitions. The partition scheme selected by PartitionFinder led to poor mixing
due to over-parametrization in initial test runs. We modified slightly the
partition scheme to use: HKY+I+G, GTR+G and HKY for each codon position
respectively for the dataset including all sequences; and HKY+G, HKY+G and HKY
for the analyses on unique haplotypes. In all analyses, base frequencies were
estimated.
...
slightly the partition scheme to improve mixing of the chains as the partition
scheme selected by PartitionFinder led to over-parametrization (supplementary
Table \ref{tab:sm-partitions}).

% need to re-word this paragraph
...
Given the shallow genetic divergence observed among some of the putative species
detected by GYMC and the lack of morphological differentiation among them, we
compared species delineation models with *BEAST v 1.7.5 \cite{Heled2010} using
...
not be distinguished from ESU1 (Table \ref{tab:delineationModels}). We included
ESU3 and gracilis in our analysis to allow the estimation of the topology when
all individuals for the species tested where attributed to ESU1. In these
analyses, we selected individuals for which we had at least one mitochondrial
marker and one nuclear marker. We used data from the following markers: 16S,
COI, ATP6, c0036, c0775, ITS, LSU and H3a; and the same approach as for the
phylogeny of the entire complex, using Gblocks to remove ambiguous parts of the
...
strict molecular clocks, fixing the COI clock at 1.85\%/million years
% Why did you use an urchin clock when you have an internal one?
\cite{Lessios2008} while estimating the rate of the others. We ran the analysis
for $50.10^{6}$ generations and repeated it at least twice for each group.
