...
data, to reveal at least 13 species within ``\textit{Holothuria impatiens}''
...
\subsection{Species delineation with GMYC}
...
sister to the one found in the Galapagos, Figure~\ref{impatiens-tree}). We
...
\subsection{Species delineation with other lines of evidence: coloration,
...
morphologically. Their ranges overlap (\texttt{Hawaii,Wpac}) or border
...
result from reproductive isolation rather than geographical structure, we
compared competing hypotheses of species delineation with *BEAST v 1.7.5
\cite{Heled2010} using Bayes Factors (BF) estimated by the stepping-stone (SS)
and the path sampling (PS) methods \cite{Baele2012}.
...
models tested with *BEAST, we repeated some of the analyses omitting COI (models
M4, M5, M6 and M7).
...
in the GMYC analyses.

To obtain a baseline of the variation that was expected in the differences in
likelihoods, we also assigned the species randomly to the individuals.
...
Galapagos, EP: Eastern Pacific; PNG: Papua New Guinea; Wpac: Western Pacific.}
\end{figure}
...
\subsection{Species delineation with other lines of evidence: morphology,
  geography, ecology}
...
least ten species (Figure~\ref{fig:impatiens-images}).
...
patterns, and their distinct geographic distributions allow to tease them
apart. \texttt{ESU2} is the only species with bright yellow papillae and
tentacles. \texttt{tiger} and \texttt{tigerRedSea} are also very distinctive
with sparse, dark, tubercles that barely raise from the body wall, giving them a
smooth appearance. They are also much larger than the other species and
specimens reach regularly more than 30~cm. \texttt{gracilis} has very large
conical, typically red tubercles and a very distinctive ``gritty'' texture. 
\texttt{ESU3} has elongated, pyramidal tubercles with white papillae.
