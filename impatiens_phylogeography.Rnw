...
data carry the signature of the historical processes experienced by species. New
coalescent-based methods using multi-locus datasets have overcome some of the
...
divergence. Not only do these methods account for the stochastic process
associated with the independent sorting of genealogies along the species tree,
but they use it to estimate the species tree and the demographic history of the
species \cite{Fujita2012a}. Additionally, these methods provide a statistical
framework to test alternate species delineation hypotheses when other lines of
evidence are equivocal \cite{Baele2012}.

Molecular methods have revealed that many species, thought to be
well-understood, are complexes of cryptic species. In many cases, these
``pseudo-cryptic'' species differ morphologically but in traits not
traditionally considered, or in traits not available to taxonomists because of
the preservation methods or the lack of information about the natural history of
the species \cite{Knowlton1993}. For instance, differences in coloration
\cite{Malay2010}, depth \cite{Prada-missing}, mating signals \cite{}, were not
considered as indicative of species limits before cryptic lineages were
...
If this approach has allowed to unravel high levels of unrecognized diversity,
when other defining traits to infer reproductive isolation are lacking, the
patterns of genetic differentiation have remained difficult to interpret
objectively. Many species rely on chemical recognition systems to maintain
species integrity (e.g., mate recognition, habitat specificity)
\cite{Knowlton1993}. Not only these systems are poorly characterized or
difficult to investigate in the context of a taxonomic study, they also rarely
translate into morphological differences. In this context, defining traits that
correlate with genetic clusters identified from single-locus data sets are
scarce, forming ``true cryptic'' species complexes.

%%% rewrite this paragraph
%% cryptic lineages paint a more complex picture of speciation than strict
%% allopatry with potential role of selection to account for rapid secondary
%% sympatry as well as changes in species distribution through time.
...
% prevalent, overlap in the distribution of sister species is common. These
% observations have led to the realization that (1) selection (along environmental
% gradients, sexual selection, host specificity) may play an important role in
% reproductive isolation (reviewed in \cite{Bowen2013}); (2) important changes in
% species ranges and distributions have obscurred the barriers responsible for the
% initiation of reproductive isolation.

In this study, using a combination of morphological, genetic and ecological
data, we show that ``\textit{Holothuria impatiens}'' represents a complex of at
least 13 species, including both ``pseudo-cryptic'' and ``true cryptic''
species: some of these species can be distinguished relatively easily from their
live appearance, others can only be identified genetically.
% And we use this radiation to investigate spatial and temporal patterns of
% diversification in the complex
...
the body wall is gray with dark spot, and with well-developed, lightly colored
tubercles. The drawing accompanying the description corroborates these
observations. Since then, the species has been attributed to
\textit{Holothuria}, the most diverse genus of the Holothuriidae, typically
reef-associated. The range of \textit{H. impatiens} has been extended, and today
is recognized as the most widespread species in the genus. Its range extends
from the Red Sea through the entire Indo-West Pacific, to the East Pacific,
...
Rowe \cite{Rowe1969} to include 13 species characterized by well developed,
regular and symmetrical tables and buttons.
...
on its reproduction \cite{Harriott1985}, Cuvierian tubules
\cite{Flammang2002,Becker+Flammang2010}; toxicity \cite{Bakus1974}; feeding
preferences \cite{Roberts1982}; parasites \cite{Martens1994}; the chemical
composition, statistical analysis of the shape, and ontogenic changes in
ossicles \cite{Hampton1958,Hampton1959,Cutress1996}. It has also been included
in molecular phylogenies that investigate relationships among the major groups
of holothurians (e.g., \cite{Lacey2005}) or as an outgroup when studying
