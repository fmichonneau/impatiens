...
\cite{Hey2010}. However, divergence in mitochondrial DNA alone is insufficient
...
mixed Yule coalescent (GMYC) model (\cite{Pons2006}, \cite{Monaghan2009a},
...
We set the MCMC chain lengths at $5.10 {7}$, sampling every $5,000$
...
estimate of the divergence times.


We fit the single-threshold (\cite{Pons2006}) GYMC model, as it was shown to
perform as well as or better than the multiple-threshold model
\cite{Fujisawa2013}.  Clusters of sequences
found beyond this threshold are considered to represent different putative
species. Singletons, unique sequences that the best fit GMYC model considered
belonging to different putative species, were assigned to the nearest
cluster. We used this conservative approach given that (1) these sequences were
not very different from the closest cluster, (2) the multispecies coalescent
used to test the species limits requires more than one individual for each
putative species \cite{Heled2010}.

The GMYC model was fit onto a the COI lineage genealogy estimated using BEAST
1.7.5. We reduced the dataset to only unique haplotypes ($n = 126$), as
identical sequences can cause issues with the parameter estimation with GMYC
\cite{Monaghan2009}. We used a strict molecular clock, specified a coalescent
constant population size tree prior, a HKY+I+G model of molecular evolution, and
ran the chains for $10^{8}$ generations. We used Tracer \cite{} to check that
the chains reached stationarity and were mixing appropriately (all ESS values
were above 100). We checked for convergence by repeating the analysis twice.

% Monaghan 2009 says constant coalescent prior is more conservative than Yule
% prior. Is it the case with my dataset?
...
The best fit GMYC model identified 13 clusters (confidence interval: 13-14) and
3 singletons. Some of these putative species are restricted geographically to an
oceanic bassin (Mediterranean, Caribbean, Eastern Pacific, Red Sea) or to an
archipelago (Hawaii and Galapagos), while other ESUs are wide ranging and are
found across the Indo-Pacific (ESU1, ESU2, and ESU3)
(Fig~\ref{fig:impatiens-map-WA}).

Most of these putative species have distinct color patterns, and the apparent
variability seen in \textit{Holothuria impatiens} is correlated with the genetic
differentiation seen in COI.

<<gmyc-coi, dependson='init-phylo', eval=TRUE, cache=TRUE, echo=FALSE, fig.keep='last', fig.height=4>>=
@ 

For single:
* most consistent estimates for strict clocks (with same estimated number of
species when considering all haplotypes or unique haplotypes)
* combination of unique haplotypes and relaxed clock lead to high uncertainty
* yule leads to most conservative estimates (There is an interaction between
tree prior and type of clock) 

Single vs multi
* CI don't overlap when using all haplotypes, but do when using only the unique
haplotypes
* multi higher than single (only one case where they are equal)
* The estimate for multi is almost always (but one case) the top-value of the CI
