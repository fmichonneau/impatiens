...
\cite{Appeltans2012,Costello2013}. DNA-based species delineation methods have
become increasingly popular as they provide an inexpensive and efficient way to
identify species and resolve complexes. These methods are efficient as genetic
data carry the signature of the historical processes experienced by species. In
...
common ancestor \cite{Hey2010-IMa2}. % lacks transition sentence here
The recent development of coalescent-based methods using multi-locus datasets
has overcome some of the shortcomings of single-locus analyses that were unable
to distinguish between some processes that lead to the same patterns.


%% GP: How do you determine the age of a fauna?  Do you mean assemblages,
%% communities, species or complexes?  Why does that lead to the two
%% consequences listed?  Do you need selection for this age?  How does age lead
%% to interpretations about distributions and barriers?
Molecular methods have revealed that many species complexes that were thought to
be well-understood are complexes of cryptic species. In particular, species
thought to be widespread are often complexes of divergent lineages with
restricted geographical ranges (e.g., \cite{Brown2007,Meyer2005-evolution},
...). Among marine organisms, these findings are changing our understanding of
speciation in the sea \cite{Mayr1954}. Since most marine invertebrates have a
highly dispersive larval stage, and the oceans in general and the Indo-Pacific
in particular lack clear geographical barriers to dispersal, opportunities for
allopatric speciation seem limited. Yet, while many closely related species have
non-overlapping distributions suggesting that allopatric speciation is
prevalent, overlap among sister species distributions is common. Recent
geological and molecular evidence suggests that most of the modern reef fauna is
recent (Miocene) \cite{Renema2008}, leading to the realization that (1)
selection (along environmental gradients, sexual selection, host specificity)
may play an important role in reproductive isolation (reviewed in
\cite{Bowen2013}); (2) important changes in species ranges and distributions
have obscurred the barriers responsible for the initiation of reproductive
isolation.
...
% GP: [You need to set the stage for the speciation-aspect of the chapter here –
% so what are you doing, why, and how will it address the problems you are
% discussing?  The introduction should focus on those aspects of the problem
% that you address in the study.]

Taxonomic studies of most marine invertebrates have traditionally relied on preserved
specimens. Preservation can render delicate morphological characters hard or
impossible to see, and are frequently lost or fade with time. Hard parts are
easily preserved and their form is generally not impacted by preservation, so
taxonomists often favor them to differentiate species. For instance, in sea
cucumbers, species-level taxonomy has almost entirely relied on the shape of
ossicles, microscopic calcareous secretions found in many tissues, because
ossicles show substantial diversity and variability, whilst variation in the
simple anatomy of these animals is limited at lower taxonomic levels. Even with
ossicles, until recent decades, workers have tended to focus only on those found
in the body wall, ignoring ossicles in internal structures, and have paid little
attention to intraspecific variation, typically illustrating the ossicle
assemblage of a single individual.

% who were the first authors to illustrate ossicles?

Forssk\r{a}l described \textit{Fistularia impatiens} from collections he made in
Suez, Egypt \cite{Forsskal1775}. His description is limited, but indicates that
the body wall is gray with dark spot and well-developed, lightly colored
tubercles are present. The drawing accompanying the description corroborates
these observations. Since then, the species has been attributed to
\textit{Holothuria}, the most diverse genus of the Holothuriidae that is
typically reef-associated. The range of \textit{H. impatiens} has been extended,
and today is recognized as the most widespread species in the genus. Its range
extends from the Red Sea through the entire Indo-West Pacific, to the East
Pacific, Caribbean and the Mediterranean.

Pearson \cite{Pearson1915} described \textit{Thymiosycia} as one of five
subgenera in \textit{Holothuria}, and designated \textit{Holothuria impatiens}
as the type species. The modern concept of \textit{Thymiosycia} was proposed by
Rowe \cite{Rowe1969} to include 13 species characterized by  well
developed, regular and symmetrical tables and buttons. 
...
under rocks in shallow waters (in particular in lagoons and back-reef habitats),
and is largely restricted to shallow water (< 10~m), although it has been
recorded down to 158~m (\cite{Samyn2014-Miriky}).  Because of its ubiquitous
distribution, it is one of the more studied species of holothuriid with studies
on its reproduction \cite{Harriott1985}, Cuvierian tubules \cite{Flammang2002},
\cite{Becker+Flammang2010}; toxicity \cite{Bakus1974}; feeding preferences
\cite{Roberts1982}; parasites \cite{Martens1994}; the chemical composition
\cite{Hampton1958}, statistical analysis of the shape \cite{Hampton1959}, and
ontogenic changes in ossicles \cite{Cutress1996}. It has also been included in
molecular phylogenies that investigate relationships among the major groups of
holothurians (e.g., \cite{Lacey2005}) or as an outgroup when studying
...
represents a complex of at least 12 putative species. Some of these species can
be distinguished relatively easily from their live appearance, others can only
be identified genetically. This, in combination with the fact that several of
these species co-occur and can share the same habitat, has
contributed to the failure to recognize this diversity, and differentiate
species in this complex.

Through broad sampling across the entire known geographic range of this complex,
we aim to:
\begin{itemize}
  \item differentiate species and assess species limits;
  \item determine morphological characters that can be used to distinguish species;
  \item reconstruct the history of diversification to understand how the
    diversity and distribution of came about.
\end{itemize}
...
differentiated using the multispecies coalescent
\cite{Heled2010,Rannala2003}. We also investigate how the general mixed Yule
coalescent model of species delineation performs with our data set, and how
sensitive this method is to the parameters used to reconstruct the phylogeny.

% TODO -- needs to include the morphological characters/traits that allow to
% distinguish species
...
(\cite{McMillan1999, Detton2007, Baldwin2012}. Given that sea cucumbers inhabit
