...
\cite{Hey2010-IMa2}. However, divergence in mitochondrial DNA alone is insufficient
to conclude for reproductive isolation. Historical interruption of gene flow
between populations might lead to genetic divergence, but without reproductive
isolation, upon secondary contact, these populations will resume
...
% present geographic distribution, only data available to investigate patterns
% of speciation when no fossils exist
...
expected increase in lineage accumulation resulting from the shift from
speciation to intraspecific coalescent events. To determine the location of the
threshold, the GMYC model is fit at different nodes along the tree, and the time
that provides the best likelihood for the location of the switch is
...
(\cite{Monaghan2009}, \cite{Fujisawa2013}), we here revisit the impact of
...
al. \cite{Monaghan2009}, indicate that using a coalescent constant tree prior
is a more conservative approach compared to a Yule prior as GMYC uses the
...
% Talavera2013 kept identical sequences

Furthermore, to our knowledge no previous study has investigated the effect of
retaining identical haplotypes in the analysis. In other studies, identical
sequences were removed as they were thought to make the task of estimating the
transition problematic \cite{Monaghan2009}, as the GMYC method cannot
accommodate for the null branch lengths associated with identical sequences
\cite{Fujisawa2013}. Contrary to other programs, BEAST does not assign zero edge
length to the terminal branches for terminal sequences, and thus allows to
retain them in the analysis. There are obvious theoretical advantages for
retaining identical sequences, as BEAST assumes that the sequences represent a
...
and HKY for for each ones of the codon position respectively for the all
dataset; and HKY+G, HKY+G and HKY for analyses on unique haplotypes. In all
...
the common ancestor tree method \cite{Heled2013} as it provides an accurate
...
Sequences obtained for each marker were aligned using MUSCLE \cite{} with the
...
(-b5=a). We used the resulting alignment to estimate the phylogenetic
relationships and the timing of the divergence among the different lineages of
\textit{Holothuria impatiens} using RAxML 8.0.1 \cite{} and BEAST 2.1.2
\cite{Bouckaert2014} (using BEAGLE 2.1.2 \cite{})
