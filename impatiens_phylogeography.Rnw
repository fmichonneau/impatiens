...
translates into a mean age of divergence set at \Sexpr{round(qlnorm(0.5,
  meanlog=1.5, sdlog=0.75) + 2.5, 2)} millions years ago (Ma), (95\% confidence
...
Caribbean to 5 Ma. The isthmus of Panama was completely closed about 3 Ma
...
1.85\%/million years \cite{Lessios2008} while estimating the rate of the
others. We ran the analysis for $50.10^{6}$ generations and repeated it at least
...
approximately 10 Ma (Fig.~\ref{fig:impatiens-tree}). Clade I has the widest
...
diverged 1.8 Ma. The three delineated species that diverged the most recently
...
whichMedit <- which(impTree$edge[ancestor(impTree4, MRCA(impTree4, c("X0002", "N0014")))
...
allopatric/peripatric speciation that is masked by the recent range expansion of
...
isolation rapidly, and contrasts with the pattern typically observed in oceanic
settings.

Instead, the temporal dynamics of diversification in some species of
the ``\textit{Holothuria impatiens}'' complex is similar to what is observed for
...
Along coastlines, the number of individuals from incipient species interacting
is much higher than in oceanic setting allowing reproductive isolation to
complete more rapidly. These interactions manifest for resource competition
