...
for the log-normal relaxed clock and the strict clock, with a $log(\text{mean})$
of $-4$ and a $log(\text{standard deviation})$ of $1$. We calibrated the COI
genealogies using the closure of the isthmus of Panama. We constrained the age
of the the node corresponding to the most recent common ancestor of the
individuals found in the Galapagos archipelago, the coast of the Eastern Pacific
and in the Caribbean using a log-normal prior with a $log(\text{mean})$ of
$1.5$, a $log(\text{standard deviation})$ of $0.75$ and an offset of $2.5$.
This distribution translates into a median age of divergence set at
\Sexpr{round(qlnorm(0.5, meanlog=1.5, sdlog=0.75) + 2.5, 2)} millions years (MY)
ago, (95\% confidence of $[$\Sexpr{paste0(round(qlnorm(c(0.025, 0.975),
