...
For the datasets used in xxx analysis, the COI alignment was xxx bp long and
included xxx variable sites, of which xxx were phylogenetic informative. 
...
<<loci-coverage, cache=TRUE, dependson='loci-characteristics-data', warning=FALSE, echo=FALSE, fig.keep='last', fig.width=6, fig.height=9.75, out.width='10cm', fig.cap='Representation of the loci sequenced for each individual included in the analyses.'>>=
...
sGMYC produced species estimates that were more conservative and with narrower
CI than the mGMYC approach. The maximum-likelihood estimates for the sGMYC were
in most cases at the lower limit of the CI, while the estimates from the mGMYC
were at the upper limit of the CI. For the analyses performed on all the
haplotypes, the CI for the estimated number of species using sGMYC and mGMYC did
not overlap.
...
The estimated number of species with sGMYC was more conservative and less
sensitive to the priors used when using all haplotypes compared to the analyses
restricted on the unique haplotypes.
...
(Figure~\ref{fig:impatiens-tree} and Supplementary Material~\ref{}), and the
...
approximately 10 MYA (Figure~\ref{fig:impatiens-tree}). Clade I has the widest
