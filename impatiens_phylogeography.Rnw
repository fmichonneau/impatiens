...
priors used. For the analyses based on all sequences, the difference in the
estimated number of species was associated with (1) the estimated time of
coalescence of divergent haplotypes related to ESU3 was occurring after the
threshold with the Yule prior; (2) the lumping of \texttt{tigerRedSea} and
\texttt{tiger}.
...
unique haplotypes and all sequences corresponds to the split of \texttt{tiger}
and \texttt{tigerRedSea}.
