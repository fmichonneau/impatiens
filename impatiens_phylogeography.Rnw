...
(morphology, ecology and geographic information) to assess the validity of these
...
\item  ``Putative species'' describes the lineages identified by the GMYC method
  and follows the terminology used by the author of the method \cite{Pons2006,
    Monaghan2009}.
\item ``Evolutionary significant units (ESUs)'' are defined as reciprocally
  ...
  as defined by Moritz \cite{Moritz1994}. The ESUs of this study include one or
  more putative species identified with GMYC.
\item The ``species'' are the lineages supported by the model of species
  ...
  exchange genes. The species include one or more ESUs.
\end{itemize}
...
of evidence. The discovery methods, based on genetic differentiation in the
mitochondrial locus COI, found putative species which status would require
additional material to be evaluated. In particular, two individuals
characterized by divergent haplotypes, related to \texttt{ESU1} and
\texttt{ESU3} respectively, were consistently recovered as separate species. In
both cases, a single individual was available, making it impossible to use
*BEAST to test for possible reproductive isolation. The individual related to
\texttt{ESU1} (UF8191) has the typical color pattern of \texttt{ESU1}, with
which it co-occurs and no line of evidence seem to indicate that it could be a
different species. The individual related to \texttt{ESU3} (RUMF-ZE-0074) is a
small individual that was dredged from 32~m. The distinct ecology and deeply
...
available (e.g., RESL \cite{Ratnasingham2013}, ABGD \cite{Puillandre2011}, bPTP
...
The multiple-threshold method (mGMYC) led to higher estimates of the number of
...
by mGMYC would split, lineages that are otherwise highly supported by the
multi-locus phylogeny. The overall lower performance of mGMYC was also observed
by the authors of the method using a simulated dataset, they however found that
the 95\% confidence set for the estimated number of putative species between
sGMYC and mGMYC were overlapping \cite{Fujisawa2013}. We observed this pattern
on the analyses using only unique haplotypes but not in the analysis that
included all sequences.
...
it was 14. These results indicate that the method is relatively robust to prior
specification. The 16 species delineation is the most realistic as it recovers
morphologically differentiated species that were not considered distinct in the
14 species hypothesis (\texttt{tiger} and \texttt{tigerRedSea}). sGMYC recovered
...
individuals will reveal rare, divergent haplotypes that may not represent
different species. However, in our analysis, it is worth noting that all our
recognized species were reciprocally monophyletic making the GMYC suitable.
...
precision of the estimates. Removing identical sequences in the analysis
overestimates the effective population size parameters that BEAST uses. In turn,
these overestimated sizes spuriously increase the time to coalescence for the
lineages, making the estimation of the inflection point between species- and
...
how many individuals need to be sampled to obtain correct estimates would be
worth pursuing.
...
other lines of evidence are equivocal (e.g., \cite{Grummer2013, Satler2013}. The
requirement to assign specimens to species \textit{a priori} can however be
challenging. When morphological evidence is scarce, and deeply divergent
lineages are recovered with genetic data, circularity is introduced when the
same data is used to recognize species, and to provide statistical support for
these species. Here, COI was used to assign species to individuals for putative
species that could not be differentiated morphologically. The results for the
analyses conducted without COI were qualitatively similar to the analyses
performed with this locus, but the statistical evidence in favor of the null
hypothesis, considering these closely related lineages as different species, was
weaker. There was no statistical evidence supporting the null hypothesis when
the analysis was conducted on nuclear data alone, owing to the low
differentiation in these loci for the putative species tested by the models.

% TODO -- need to make a drawing to clarify what I want to say here
% This suggests that until many additional nuclear markers are sampled to
% counter-balance the information lost by ignoring mitochondrial data,
