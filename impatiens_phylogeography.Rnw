...
nucleotide positions in the alignment) contained heterozygous positions. We used
the International Union of Pure and Applied Chemistry (IUPAC) nomenclature codes
for these positions in the alignment, which are considered as missing data by
all the software used in this study.
...
mitochondrial marker and one nuclear marker (\Sexpr{length(sBeastTax)}
...
x\texttt{ESU3}). We used data from the following markers: 16S, COI, ATP6, c0036,
...
divergent haplotypes related to ESU3 with the Yule prior.
...
related to \texttt{ESU3} (RUMF-ZE-0074) is small individual, and was dredged
from 32~m in a soft bottom bay of Kume Island, Ryukyus, Japan, a different
habitat from the typically reef-asssoicated \texttt{ESU3}. The distinct ecology
and deeply divergent sequences characterizing this individual suggest it could
represent a different species but additional material is needed to assess this
hypothesis.
...
difficult to generalize as they depend in part on the nature of the
dataset. Other species delineation methods using sequence data are also
available (e.g., RESL \cite{Ratnasingham2013}, ABGD \cite{Puillandre2012a}, bPTP
\cite{Zhang2013}, bGMYC \cite{Reid2012}), and it would be interesting to
...
sGMYC recovered
more putative species than we can confidently recognize, since they were
represented by single individuals. This is a common pitfall of single-locus
species delineation methods, where the broad geographical sampling of many
individuals will reveal unique, divergent haplotypes that may not represent
different species. However, in our analysis, it is worth noting that all our
recognized species were reciprocally monophyletic making the GMYC approach
suitable.
...
association between color patterns and species limits is maintain by assortative
...
The estimated ages of the \texttt{Medit}, and the two Red Sea species are older
...
these seas recently and found refugia at the periphery of these basins
during the salinity crises \cite{DiBattista2013}. It is also interesting to note
that the age of divergence for the two species from the Red Sea do not coincide
