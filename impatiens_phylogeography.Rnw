...
number of species corresponds to minimum and maximum number of species for
...
with a proportion of invariant sites and/or Gamma distributed rates across
sites). We selected the best-fit models and partition schemes based on the
...
Table \ref{tab:sm-partitions}). We used the closure of the isthmus of Panama to
calibrate our phylogeny, and fixed the time of divergence for the individuals of
the Eastern Pacific (including Galapagos) and the Caribbean to 5 MY. The isthmus
of Panama was completely closed about 3 MY ago but most species were probably
isolated earlier \cite{Lessios2008}. We chose a conservative estimate to account
for the fact that the sister species lineages found across the isthmus are the
lineages found in the Galapagos and the Caribbean (the coastal Eastern Pacific
lineage is sister to the one found in the Galapagos,
Figure~\ref{impatiens-tree}), suggesting that the divergence might have been
preceded the closure of the isthmus. Additionally, the estimated age of this
node across the COI genealogies used for the GMYC analyses, varied between
\Sexpr{round(min(meanTmrca), 2)} and \Sexpr{round(max(meanTmrca), 2)} MY
(estimated from the mean on the posterior distributions, prior
\Sexpr{round(exp(1.5 + (.75^2)/2) + 2.5, 2)} MY; median ranged between
\Sexpr{round(min(medianTmrca), 2)} and \Sexpr{round(max(medianTmrca), 2)} MY,
prior \Sexpr{qlnorm(.5, meanlog=1.5, sdlog=.75) + 2.5} MY).
...
starting tree. The Markov chains were run for $150.10^6$ generations (and
...
We used Tracer 1.6 to check that the MCMC chains had reached stationarity and
that the two independent runs were consistent. We ensured that enough of the
posterior distributions for each parameter was sampled by checking that the ESS
values for all parameters were above 200 (with a 10\% burnin) for each
independent run, the trees sampled were combined.
...
analysis for $5.10^{8}$ generations and repeated it twice for each group.
...
100, and sampling along evenly spaced quantiles of a $Beta(0.3, 1)$
distribution. However, given the differences observed in the marginal likelihood
estimations between the independent runs, we ran the analysis again with chain
lengths of $3.10^{6}$.
...
Members of the impatiens group are usually characterized by having two main
colors, a darker one and a lighter one, that vary in their hues across species
and across individuals. The dark color is typically brown but varies in
lightness and can take a red or purple tint. The lighter color is typically gray
but varies in lightness from light brown to yellow or even white. The
light-colored areas, are also typically harboring a ``salt-and-pepper'' pattern
with small spots of a darker coloration on the light background.
...
tubules, rarely released, few in numbers, long, relatively wide (about 2mm in
diameter), white with a light blue tint.
