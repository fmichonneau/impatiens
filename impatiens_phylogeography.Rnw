...
{\centering \includegraphics[width=\maxwidth]{figures-remake/gmyc-coi-plot}

}

\caption[Number of species estimated by single and multi-threshold GMYC methods
on COI phylogenies]{Number of species estimated by single and multi-threshold
  GMYC methods on COI phylogenies. The dots represent the number of species
  associated with the node(s) that correspond(s) to the highest likelihood(s)
  for the location of the shift(s), the dotted lines represent the range for the
  estimated number of species for nodes within 2 likelihood units of the
