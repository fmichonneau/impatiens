...
Bayes Factors evaluated by the stepping-stone (SS) and path sampling (PS)
...
isolation (absence of gene flow) among the species and the ability to assign
...
\cite{Baele2012}. Kass \& Raftery \cite{Kass1995} adapted guidelines to
interpret the BF values such that $2 ln(B_{01})$ must be $> 6$ to consider the
evidence ``strong'' and $> 10$ to consider it ``very strong''. In this paper, we
use the path-sampling (PS, \cite{}) and stepping-stone (SS, \cite{}) methods to
estimate the BF as implemented by Baele \cite{Baele2012}. These methods have
been demonstrated to be much more accurate than other alternatives.
...
\begin{table}
  ...
  \begin{tabular} { l l l }
    \hline
    Model &     Number of species  &  ESUs considered a single species with ESU1 \\
    \hline
