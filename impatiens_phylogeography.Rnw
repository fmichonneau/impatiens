...
has allowed the discovery of many species complexes. Ideally, once unraveled,
diagnostic characters other than genetics or internal morphological traits
should be able to be used to tease apart the newly identified forms, so that
they can be identified in the field. The genetic differentiation observed in
species previously thought to be found across large geographic areas (e.g,
circum-tropical species) has changed our understanding of how biodiversity is
distributed in the sea, with isolated oceanic bassins and archipelagoes
exhibiting much higher endemism rates than previously thought
(e.g., %find references for Caribbean, Red Sea, Medit
...
poorly understood morphological variation that is seen in the \textit{Holothuria
  impatiens} complex. To this end, we delineated species within the
\textit{Holothuria impatiens} complex using (1) a threshold approach on a
Neighbor-Joining tree, and (2) the general mixed Yule-coalescent (GMYC) model on
a tree obtained using Bayesian inference with BEAST \cite{}. To compare how COI
performs in these genetic based delineation methods, we repeated this approach
using all mitochondrial data, all the nuclear data, and on the combined nuclear
and mitochondrial data. This latter tree was also used to assess the
diversification history in the complex.
...
process, a GTR+I+G model of molecular evolution, and ran the chains for $10^6$
generations. 
...
*BEAST infers the species trees from a multi-locus datasets by incorporating the
...
isolation (absence of gene flow) among the species and requires to assign
...
complexity of the models are directly accounted for by the marginal likelihoods
\cite{Baele2012}. Kass \& Raftery \cite{Kass1995} developed guidelines to
interpret BF values such that $2.ln(B_{01})$ (where $B_{01} =
\frac{p(Y|M_1)}{p(Y|M_0)}$ with $Y$ is the observed data and $M_i$ the model under
consideration) must be $> 6$ to consider the evidence ``strong'' and $> 10$ to
consider it ``very strong''. In this paper, we use PS and SS methods to estimate
and compare the marginal likelihoods of alternative assignments of individuals
to species in *BEAST. With this approach, we test whether we can detect signal
of reproductive isolation among individuals that cannot be teased apart reliably
based on morphology alone.

We investigated whether *BEAST favored models where all or some of the following
ESUs were considered as distinct species: ESU1, WPac, Hawaii, RedSea, and
gracilis; and used ESU2 and ESU3 as outgroups (Table
\ref{tab:delineationModels}). In these analyses, we selected individuals for
which we had at least one mitochondrial marker and one nuclear marker. We used
data from the following markers: 16S, COI, ATP6, c0036, c0775, ITS, LSU and
H3a; and the same approach as for the phylogeny of the entire complex, using
Gblocks to remove ambiguous parts of the alignment in ITS, PartitionFinder to
find the most appropriate partition scheme and models of molecular evolution. We
unlinked clocks for all markers, used strict clocks, fixing COI clock at 3.7\%
while estimating the rate of the others. We ran the analysis for $10^8$ steps and
repeated it twice for each combination. % I hope I'll have time to do that!
...
\begin{table}
  ...
  \begin{tabular} { l l l }
    \hline
    Model &     Number of putative species  &  ESUs considered a single species with ESU1 \\
    \hline
    M0    &     1                  &  WPac, Hawaii, RedSea, gracilis \\
...
<<impatiens-tree, eval=TRUE, cache=TRUE, echo=FALSE, fig.keep='last', fig.height=8, fig.cap="Bayesian phylogenetic tree of the \\textit{Holothuria impatiens} complex. Vertical bars along the tips of the tree show the different ESUs found in the complex. Axis on the bottom in My.">>=
@
...
<<impatiens-map-WA, eval=TRUE, cache=TRUE, echo=FALSE, fig.keep="last", message=FALSE, warning=FALSE, results="hide", fig.cap="Distribution maps of the ESUs.">>=
@ 

<<impatiens-map-group1, eval=TRUE, cache=TRUE, echo=FALSE, fig.keep="last", message=FALSE, warning=FALSE, results="hide", fig.cap="Distribution map for the ESUs in group 1.">>=
@ 

<<impatiens-map-group2, eval=TRUE, cache=TRUE, echo=FALSE, fig.keep="last", message=FALSE, warning=FALSE, results="hide", fig.cap="Distribution map for the ESUs in group 2.">>=
@ 
