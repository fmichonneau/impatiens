...
## 20140721 -- starBEAST
* to see if changing the molecular clock affects the likelihood calculations, I
  resubmitted the analysis for the entire complex, using clock of 0.01 for COI
  (which is what was estimated by the GMYC analyses using all sequences and the
  prior on the isthmus).

## 20140724 -- starBEAST
* yes, it changes the likelihood went from -17428 to -17438. To see if it
  affects the BF for model comparisons re-ran noWpac, noRedSea and noHawaii (2
  replicates each) on CIPRES

## 20141104 -- starBEAST (test of STACEY)
* STACEY works with BEAST2 so downloaded v2.1.3
* installed STACEY in ~/.beast/2.1/STACEY
* followed instructions from STACEY manual
  - file > templates > STACEY
  - import alignment (used /home/francois/Documents/Impatiens/000.impatiens_datafiles/starBeast_esu1_allDNA/20130930.impatiens_starbeast.nex)
  - trees:
    - mtDNA_tree for 16S, ATP6, COI
    - rDNA_tree for ITS, LSU
    - H3a_tree for H3a
    - c0036
    - c0775
  - see below for model of molecular evolution used
  - see below for clocks used
* models used
  | partition | loci                        | model    |
  |-----------|-----------------------------|----------|
  | p1        | 16S, ATP6\_2                | GTR+G    |
  | p2        | ATP6\_1, COI\_2             | HKY+I    |
  | p3        | ATP6_3, c0775, COI\_3       | HKY+I    |
  | p4        | c0036                       | TN93+I+G |
  | p5        | COI\_2, H3a\_1, H3a\_2, LSU | HKY+I    |
  | p6        | H3a_3                       | TN93     |
  | p7        | ITS                         | HKY+G    |
* clocks used
  - 16S, ATP6\_clock, c0036, c0775, COI\_clock, H3a\_clock, rDNA\_clock
* Priors
  - COI\_clock set as uniform prior on 0.0184-0.0186
  - all other clocks using exponential pr

## 20141117 -- starBEAST (test of STACEY)

After a few complicated runs (mostly due to bug in beauti assigning correct parameters with clock encompassing several nucleotide partitions), tweaked priors on molecular evolution rates based on initial run.
submitted with 20141117.stacey_allESUs prefix

## 20141119 -- starBEAST (with STACEY)

results from 20141117 show better mixing but incorrect phylogeny seems supported
with gracilis sister to RedSea + Hawaii. However, stacey supports the prior
grouping majoritarily.

Changing a few things:
- in site model: checked fix mean subst. rate, and all subst. rate are checked
  for 'estimate'
- in clock rate, mtDNA_clock is unchecked for estimate, but fixed at 0.0185
- add additional "oversplit" population

## 20141121 -- starBEAST (with STACEY)

Something is wrong and the parameters for the clock/subst rates are not
identifiable. Reading again a message from the mailing list makes think that the
subst. rate isn't needed and can be fixed to one, while the clock rate should be
estimated. I fixed all subst. rates to 1 (unchecked estimate); while I set the
clocks to be estimated (except mtDNA_clock that is set to 0.0185).

## 20141125 -- starBEAST (with STACEY)

Good mixing and ESS for previous run. Stacey favors model with ghost population,
running again but removed ghost population. Depending on result, will email
author of Stacey for suggestions.
...
# 20150903 -- Test Yule calibrated

## Rationale

After reading a post on the BEAST mailing list, it seems that Yule calibrated
is needed if tree contains calibrated nodes. Testing impact on results, by
running analysis with all sequences, relaxed clock and calibrated Yule prior.

## Methods

* Because the XML files were created with BEAST 1.8.0, I had to re-create the
  XML file with beauti. Imported nexus file from a paritition finder analysis.
* submitted to CIPRES (sinqueso) with job limit at 50 hours

## Results

* ESS values not so good. Likelihood OK (> 700) but posterior, prior and others
  pretty low.
  - the tmrca for the EP+WA+Gala is off, BEAST didn't seem to take into
    consideration the offset for the prior (listed correctly in one place of the
    file but not the other)
	- strong correlation between tmrca,rootHeight,yule.BirthRate,clock parameters.

## Next analysis

- use simpler models of molecular evolution
- make sure the offset is properly written in input file

# 20150904 -- Test Yule calibrated (2)

## Methods

- Use HKY+G for first 2 codon positions, HKY for 3rd
- calibration of tmrca: mean in *real space* of 5, log(SD) 1, offset 2.5
- calibration for clock (relaxed): lognormal mean at -4, SD at 1

## Results

- Overall, still pretty poor mixing/no convergence.
- It seems that estimating both a relaxed clock and an accurate speciation rate
  for the Yule calibrated distribution is difficult with just COI. Alternative
  hypothesis, too much information being used to estimate parameters from all
  the COI partitions.

## Next analysis

- To attempt to detect whether the lack of convergence/poor mixing is caused by
  the interaction between relaxed clock & Yule calibrated or because of
  overparametrization with too many partitions, 2 analyses:
  * One with a single HKY+G for all codon positions
  * One with strict clock

# 20150907 -- Test Yule calibrated (3)

## Methods for allSeq\_strict\_calYule

- Use HKY+G for first 2 codon positions, HKY for 3rd
- calibration of tmrca: lognormal distribution with mean in *real space* of 5,
  log(SD) of 1, offset 2.5
- calibration for clock (strict): lognormal distribution log(mean) at -4, SD at
1

## Methods for allSeq\_noCodon\_relaxed\_calYule

- Use HKY+G for all codon positions
- calibration of tmrca: lognormal distribution with mean in *real space* of 5,
log(SD) of 1, offset 2.5
- calibration of lognormal relaxed clock: lognormal distribution log(mean) at
-4, log(SD) 1

## Results allSeq\_strict\_calYule

- Really good mixing, all ESS > 1000

## Results allSeq\_noCodon\_relaxed\_calYule

- poor mixing/convergence, low ESS for the usual parameters

## Conclusions

- Estimating both the parameter of the calibrated Yule and the relaxed clock is
too much for just COI.
- The posterior distrubtions for the tmrca are identical in both cases (not
  surprising given that the only amount of information that is being used to
  calibrate tree/clock)
- The posterior distributions for the total age of the tree is shifted towards
  older ages for the strict clock (and is probably more realistic as it
  estimates it at ~10 Ma while the relaxed clock puts the median around ~5 Ma)
- It now remains to know if using calibrated Yule instead of Yule influences the
results/timing on the tree.

## Next step

- Running analysis with exactly same parameters/priors as
  allSeq\_strict\_calYule but with using Yule prior for the tree.

# 20150908 -- Test Yule calibrated (4)

## Methods

Same as above but using Yule prior instead of Calibrated Yule.

## Results

Good mixing/convergence. Very similar results. The posterior distribution for
the tmrca(EP+WA+Gala) seems to be closer than the prior for Calibrated Yule than
for Yule.

The posterior for rootHeight is a little tighter and the mean/median suggest a
younger radiation with Calibrated Yule.

The clock.rate estimates are however almost completely identical (a little
faster for Calibrated Yule at 0.0095 (compared to 0.00907)
...
* redoing it without outgroup, has long S0213 branch could lead to suboptimal
  likelihood results
* all results discarded
* resubmitting with same prefix

# 20140721 -- BEAST on full complex

* To make things more consistent in the paper, used 20140519 as starting point,
  but changed calibration of WA+EP+Gala node to the same as for the GMYC
  analyses on COI alone:
  - used a log-normal with mean 1.5, sd 0.75 and offset of 2.5.
  - I do not expect much difference
